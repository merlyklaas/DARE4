...
pacman::p_load(rio, here, tidyverse, DT, ggplot2, xaringan, knitr, kableExtra, modelsummary, stargazer, xaringanthemer, gganimate, ggthemes, fixest, haven, arsenal, MatchIt, gtools)
...
```{r}
table <- tableby(elprgm ~ race + ses + chrabsk + prelas + ebrs + female + rural, 
                 numeric.stats=
                   c("meansd"), cat.stats=c("N", "countpct"), digits=2, data=dat)
mylabels <- list(faminc8 = "", 
                 math8 = "")
...
pscore_df <- data.frame(p_score = predict(pscores, type = "response"),elprgm = dat$elprgm)
...
ggplot(pscore_df, aes(p_score,fill = as.factor(elprgm), color = as.factor(elprgm))) + 
  ...
  xlab("Probability of being clasified as EL")
```
...
matched <- matchit(elprgm ~ prelas + ebrs + kmath + kread + ses + rural + female + hisp, 
                   method="nearest", 
                   replace=T, 
                   discard="both", 
                   data=dat)
...
ggplot(df_match, aes(distance,fill = as.factor(elprgm), color = as.factor(elprgm))) + 
        ...
        xlab("Probability of Being Classified as English Learner")
...
psmatch1 <- lm(tlangk ~ elprgm + prelas + ebrs + kmath + kread + ses + rural + female + hisp, data=df_match, weights=weights)
psmatch2 <- lm(tmathk ~ elprgm + prelas + ebrs + kmath + kread + ses + rural + female + hisp, data=df_match, weights=weights)
