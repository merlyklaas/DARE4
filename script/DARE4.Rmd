...
```{r}
table_diff <- tableby(elprgm ~ hisp + ses + chrabsk + female + rural + ebrs + prelas, 
...
# Median Cut-point for executive function
summary(dat$kexecfunc1) #median = 15
...
cem <- matchit(elprgm ~ prelas_quints + ebrs_quints + ses_quints + rural + female + hisp, method="cem",
               data=dat)
...
```{r}
#estimation for teacher perception on language skills
att1 <- lm(tlangk ~ elprgm + prelas + ebrs+ ses+ kexecfunc1 + rural + female + hisp, data=df_cem, weights = weights)

#estimation for teacher perception on math skills
